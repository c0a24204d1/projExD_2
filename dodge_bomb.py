...
def gameover(screen: pg.Surface) -> None:
    ...
    txt_rct = txt.get_rect() #文字Surfaceに対応する文字Rectを取得する
    txt_rct.center = WIDTH/2, HEIGHT/2 #文字の座標を中央に設定する
    ff_img = pg.image.load("fig/8.png") #画像を挿入する
    ...
    screen.blit(ff_img, ff1_rct) #設定した座標(右)に貼り付ける
    screen.blit(ff_img, ff2_rct) #設定した座標(左)に貼り付ける
    pg.display.update()
...
def init_bb_imgs() -> tuple[list[pg.Surface], list[int]]:
    ...
    for r in range(1, 11):              #拡大爆弾surfaceのリスト
        ...
        bb_imgs.append(bb_img) #リストに追加する
...
def main():
    ...
    while True:
        ...
        bb_rct.move_ip(avx, vy) #爆弾の移動 avxでvxの設定
